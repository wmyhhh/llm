...
"""
使用 EETQ 量化 Qwen/Qwen2.5-3B 模型的示例代码
"""
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='使用EETQ量化运行Qwen2.5-3B模型')
    parser.add_argument('--bits', type=str, choices=['int8'], default='int8',
                        help='量化位数: 目前EETQ仅支持int8')
    ...
    # 配置EETQ量化参数 - 目前EETQ仅支持int8
    eetq_config = EetqConfig(args.bits)
    
    # 记录加载开始时间
    load_start_time = time.time()
    
    # 加载并量化模型
    print(f"开始使用EETQ进行{args.bits}量化...")
    ...
    try:
        ...
        print("\n成功加载 EETQ 量化模型 (int8)")
        model_description = "EETQ INT8 量化模型"
    except Exception as e:
        print(f"\n警告：EETQ 量化加载失败，尝试使用默认设置: {str(e)}")
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            trust_remote_code=True,
            device_map="auto",
        ...
        model_description = "EETQ 量化失败，使用默认 FP16 模型"
    ...
    if args.save_results:
        ...
        results_file = os.path.join(results_dir, f"eetq_performance_test_{timestamp}.json")
    ...
    if save_model:
        save_path = f"qwen2.5-3b-eetq-{args.bits}"
