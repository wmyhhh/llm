...
"""
加载预训练的 AQLM 量化模型并执行性能分析
支持加载 Hugging Face 上的 ISTA-DASLab/Llama-2-7b-AQLM-2Bit-1x16-hf 模型
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='加载AQLM预量化模型并执行性能分析')
    parser.add_argument('--model_name_or_path', type=str, default="ISTA-DASLab/Llama-2-7b-AQLM-2Bit-1x16-hf",
                        help='模型路径或Hugging Face模型标识符')
    parser.add_argument('--device', type=str, default="cuda:0",
                        help='加载模型的设备')
    parser.add_argument('--codebook_size', type=int, default=16,
                        help='仅用于统计和显示，不影响实际模型加载')
    parser.add_argument('--num_codebooks', type=int, default=1,
                        help='仅用于统计和显示，不影响实际模型加载')
    parser.add_argument('--save_results', action='store_true',
                        help='是否保存性能测试结果')
    parser.add_argument('--pre_quantized', action='store_true', default=True,
                        help='预量化模型标志(已默认启用)')
    parser.add_argument('--output_dir', type=str, default="./llama2_aqlm_loaded",
                        help='保存模型的目录')
...
def main():
    ...
    # 打印正在加载的模型信息
    print(f"正在加载预训练AQLM模型: {model_name}")
    ...
    # 由于我们加载的是预量化模型，确保设置pre_quantized=True
    if not args.pre_quantized:
        print("警告: 您正在加载的是预量化AQLM模型，自动启用pre_quantized=True")
        args.pre_quantized = True
    
    # 记录加载开始时间
    load_start_time = time.time()
    
    # 加载模型 - 对于预量化的AQLM模型，尽量简化加载参数，避免冲突
    try:
        print("加载模型中，请耐心等待...")
        ...
        try:
            ...
            print(f"再次尝试加载模型失败: {e2}")
            print("\n可能的解决方案:")
            print("1. 确保已安装所有必要的依赖: pip install transformers==4.36.0 optimum aqlm")
            print("2. 检查模型路径是否正确")
    ...
    # 构建模型描述 - 这里不使用后端信息，因为我们直接加载预训练模型
    model_description = f"AQLM 2bit量化模型 (Llama-2-7b)"
    
    # 运行对话循环
    print("\n" + "="*50)
    print(f"{model_name} {model_description}已加载完成。开始性能评估测试。")
    ...
    # 性能评估
    test_question = "How to calculate the determinant of a matrix?"
